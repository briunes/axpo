 ...
 *
 * Generates OpenAPI 3.0 schema from JSDoc comments in route files
 */
...
const apiFileGlobs = [
  ...
  path.join(process.cwd(), "backend", "app", "api", "**", "*.ts"),
];
...
Secure backend API for AXPO simulation management with role-based access control,
public token+PIN sharing, base values versioning, and audit logging.

## Features
- RBAC (Admin/Agent/Commercial)
- Secure public access (token+PIN)
...
## Authentication
- **Internal APIs** (\`/api/v1/internal/*\`): Bearer JWT token
- **Public APIs** (\`/api/v1/public/*\`): Public token + PIN (4 digits)
...
\`\`\`
    ...
    servers: [
      {
        url: "/api/v1/internal",
        description: "Internal API (requires authentication)",
      },
      {
        url: "/api/v1/public",
        description: "Public API (token+PIN)",
      },
    ],
    components: {
      securitySchemes: {
