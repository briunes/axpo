...
const STATUSES: SimulationIssueItem["status"][] = ["NEW", "IN_REVIEW", "ESCALATED", "RESOLVED", "DISMISSED"];
...
export default function SimulationIssueDetailPage() {
  ...
  const [draftAppStatus, setDraftAppStatus] = useState<SimulationIssueItem["appStatus"]>(null);
  ...
  const isAppIssue = issue?.status === "ESCALATED";
  ...
  const mutation = useMutation({
    mutationFn: () => updateSimulationIssueStatus(session!.token, id, selectedStatus, notes, isAppIssue && canManageAppIssue && draftAppStatus !== null ? selectedAppStatus : undefined),
    onSuccess: (updated) => {
      ...
      setDraftStatus(null); setDraftAppStatus(null); setNotes(""); showSuccess(t("simulationIssues", "updateSuccess"));
    },
  ...
  const statusColor = issue.status === "NEW" ? "error" : issue.status === "ESCALATED" ? "info" : issue.status === "IN_REVIEW" ? "warning" : issue.status === "RESOLVED" ? "success" : "default";
  const issueFiles = [
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    if (!needsNotes && hasChanges) mutation.mutate();
  };
  ...
  return <CrudPageLayout title={t("simulationIssues", "issueDetails")} backHref="/internal/simulations/issues" maxWidth={undefined} hideHeader>
    <Box className="crud-tab-panel" data-tour="simulation-issue-detail-page" sx={{ position: "relative", p: { xs: 2, md: 2 } }}>
      ...
      <Box sx={{ display: activeTab === 0 ? "block" : "none" }}>
        <CrudFormContainer variant="plain" onSubmit={handleSubmit} submitLabel={t("simulationIssues", "saveChanges")} cancelLabel={t("actions", "cancel")} onCancel={() => router.push("/internal/simulations/issues")} isSubmitting={mutation.isPending} disableSubmit={needsNotes || !hasChanges} onRenderActions={setFormActions}>
          <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "repeat(2, minmax(0, 1fr))", lg: "repeat(4, minmax(0, 1fr))" }, gap: 2, alignItems: "start" }}>
            ...
            {isAppIssue ? <FormSelect label={t("simulationIssues", "appStatus")} options={STATUSES.filter((status) => status !== "ESCALATED").map((status) => ({ value: status, label: labels[status] }))} value={selectedAppStatus} onChange={(value) => setDraftAppStatus(value as SimulationIssueItem["appStatus"])} disabled={mutation.isPending || !canManageAppIssue} /> : <FormSelect label={t("simulationIssues", "changeStatus")} options={STATUSES.map((status) => ({ value: status, label: labels[status] }))} value={selectedStatus} onChange={(value) => setDraftStatus(value as SimulationIssueItem["status"])} disabled={mutation.isPending} />}
          </Box>
          ...
          <Box sx={{ display: "grid", gridTemplateColumns: savedStatusNotes.length ? { xs: "1fr", lg: "minmax(0, 1fr) minmax(0, 1fr)" } : "1fr", gap: 2, alignItems: "start" }}>
            <FormInput label={isEscalating ? t("simulationIssues", "escalationNotesRequired") : isChangingToResolved ? t("simulationIssues", "resolutionNotesRequired") : t("simulationIssues", "statusNotes")} value={notes} onChange={(event) => setNotes(event.target.value)} multiline required={isChangingToResolved || isEscalating} error={needsNotes} helperText={isEscalating ? t("simulationIssues", "escalationNotesHelp") : needsNotes ? t("simulationIssues", "resolutionNotesHelp") : undefined} disabled={mutation.isPending} sx={{ "& .MuiInputBase-root.MuiInputBase-multiline": { height: 360, alignItems: "flex-start" }, "& textarea": { height: "100% !important", maxHeight: "100% !important", overflowY: "auto !important", resize: "none" } }} />
