...
export function SimulationIssuesPanel({ session, onNotify }: { session: SessionState; onNotify?: (text: string, tone: "success" | "error") => void }) {
  ...
  const columns = useMemo<ColumnDef<SimulationIssueItem>[]>(() => [
    ...
    { key: "incidentNumber", label: t("simulationIssues", "incidentNumber"), minWidth: 130, flex: 0.5, renderCell: (row) => <Link component={NextLink} href={`/internal/simulations/issues/${row.id}`} underline="hover" sx={{ fontWeight: 600 }}>#{row.incidentNumber}</Link> },
